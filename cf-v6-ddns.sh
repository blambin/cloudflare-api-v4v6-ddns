...
CFUSER=xxx@gmail.com

# Zone name, eg: example.com
CFZONE_NAME=xxx.org

# Hostname to update, eg: homeserver.example.com
CFRECORD_NAME=xxx.xxx.org
...
WAN_IP_FILE=$HOME/.cf-wan_ipv6_$CFRECORD_NAME.txt
...
ID_FILE=$HOME/.cf-idv6_$CFRECORD_NAME.txt
if [ -f $ID_FILE ] && [ $(wc -l $ID_FILE | cut -d " " -f 1) == 4 ] \
  ...
  && [ "$(sed -n '4,1p' "$ID_FILE")" == "$CFRECORD_NAME" ]; then
    ...
    CFRECORD_ID=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$CFZONE_ID/dns_records?name=$CFRECORD_NAME" -H "X-Auth-Email: $CFUSER" -H "X-Auth-Key: $CFKEY" -H "Content-Type: application/json"  | jq -r '.result | .[] | select(.type == "AAAA") | .id ')
